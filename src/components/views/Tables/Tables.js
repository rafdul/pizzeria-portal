...
// function createDataTables(id, time, table1, table2, table3, event){
//   return {id, time, table1, table2, table3, event};
// }
...
class Tables extends React.Component {
  static propTypes = {
    fetchBookingsAndEvents: PropTypes.func,
    loading: PropTypes.shape({
      active: PropTypes.bool,
      error: PropTypes.oneOfType([PropTypes.bool,PropTypes.string]),
    }),
    // bookedTable: PropTypes.array,
    bookedTable: PropTypes.oneOfType([
      PropTypes.object,
      PropTypes.array,
    ]),
  }
  ...
  componentDidMount(){
    const { fetchBookingsAndEvents } = this.props;
  ...
  render() {
    const { loading: { active, error }, bookedTable } = this.props;
    // console.log('bookedTable', bookedTable);

    if(active || !bookedTable.length) {
      return(
                  ...
                  <TableBody>
                    {bookedTable.map( row => (
                      <TableRow key={row.id}>
                        ...
                        <TableCell component='td' >{row.time}</TableCell>
                        <TableCell component='td' >
                          {/* {row.table1} */}
                          {this.addBooking(row.table1)}
                          {/* {row.table1 && (
                        ...
                        <TableCell component='td' >
                          {/* {row.table2} */}
                          {this.addBooking(row.table2)}
                        </TableCell>
                        <TableCell component='td' >
                          {/* {row.table3} */}
                          {this.addBooking(row.table3)}
                        </TableCell>
                        <TableCell component='td' >
                          {/* {row.event} */}
                          {this.addEvent(row.event)}
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
